 ...
 * @env_list: pointer to env_list linked list
 *
...
int locate_path(char *path, env_ment *env_list)
{
	...
	while (tmp)
	{
		if (_strg_com_pare(tmp->var, "PATH") == 0)
		{
			_str_co_py(path, tmp->val);
			return (1);
		}
...
int cat_path(char **search_path, char *cmd)
{
	...
	new = safe_malloc(BU_FFER_SIZE);

	for (i = 0; search_path[i] != NULL; i++)
	{
		new = _str_co_py(new, search_path[i]);
		len = _str_length(new);
		new[len] = '/';

		new = _strn_cat(new, cmd, _str_length(cmd));
		fd = access(new, F_OK);
		if (fd == 0)
		{
			_str_co_py(cmd, new);
		...
		{
			new = mem_reset(new, BU_FFER_SIZE);
		}
	}
...
/**
 * gen_token_path - gen_tokenr specifically for path
	...
	while (1)
	{
		if (path_str[i] == ':' || path_str[i] == '\0')
		{
			...
			if (end == start)
			{
				...
				_str_co_py(paths[count], "./");
			}
			else
			{
				paths[count] = safe_malloc((end - start + 1) * sizeof(char));
				_strng_co_py(paths[count], &path_str[start], end - start);
			}
			count++;
			start = i + 1;
		}
